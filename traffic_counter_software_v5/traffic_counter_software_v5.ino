...
int trigger_value2;
int threshold = 4; //change this amount if necessary. tunes sensitivity.
...
void setup() {
  ...
  trigger_value2 = analogRead(A1) + threshold;
  delay(1000);
  Serial.println("Hello, Welcome to the DIY Traffic Counter");
  Serial.println("Developed by Tomorrow Lab in NYC, ");
    Serial.println("Updated by Roy Stillwell, Colorado School of Mines 2013 ");
  ...
  Serial.println(trigger_value2 - threshold);
  Serial.println("___________________________________________________");
  Serial.println("");
...
void loop() {
 ...
 //1 - TUBE 1 IS PRESSURIZED INITIALLY
  //Egress
  if (analogRead(A0) > trigger_value) {
    if (car_out = 0 && car_in = 0 && strike_number == 0) { // FIRST HIT
      Serial.println("");
  ...
  //Ingress
    if (analogRead(A1) > trigger_value2) {
  ...
  while(analogRead(A0) > the_max && is_measuring == 1) { //is being pressed, in all cases. to measure the max pressure.
    ...
    while(analogRead(A1) > the_max2 && is_measuring == 1) { //is being pressed, in all cases. to measure the max pressure.
    the_max2 = analogRead(A1); 
  }
